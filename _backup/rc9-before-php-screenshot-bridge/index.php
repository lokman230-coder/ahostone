...
// v25.0.0 RC8 - Unified QA & Scan Center Pro (visual + system scan)
...
if (in_array($route, ['admin/qa-scan-center/run','admin/qa-visual-scan/run','admin/scan-report/run'], true)) {
    ...
    try {
        ...
        // PDF eklendikten sonra ZIP paketini tekrar oluştur.
        QAScanCenterService::createReport($base, $systemScan); // yeni timestamp istemediğimiz için aşağıda mevcut zip korunur; HTML/JSON zaten hazır.
        // Mevcut klasörde PDF eklendikten sonra hafif zip güncellemesi için servis içi paketleyici yerine CLI varsa zip kullanılır.
        if (is_file($dir.'/qa-scan-package.zip')) @unlink($dir.'/qa-scan-package.zip');
        if (class_exists('ZipArchive')) { $zip=new ZipArchive(); if($zip->open($dir.'/qa-scan-package.zip', ZipArchive::CREATE|ZipArchive::OVERWRITE)===true){ $it=new RecursiveIteratorIterator(new RecursiveDirectoryIterator($dir, FilesystemIterator::SKIP_DOTS)); foreach($it as $f){ if($f->getFilename()==='qa-scan-package.zip') continue; $zip->addFile($f->getPathname(), str_replace('\\','/', substr($f->getPathname(), strlen($dir)+1))); } $zip->close(); } }
        flash('success', 'QA & Scan tam tarama tamamlandı. Rapor paketi oluşturuldu: '.basename($dir));
    } catch (Throwable $e) {
