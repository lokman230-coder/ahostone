<?php
/**
 * Ahost One v25.0.0 RC20 - Kurulum
...
// RC20 güvenlik: mevcut config doluysa kurulumun canlı sistemi yanlışlıkla geriye çekmemesi için uyarı verilir.
$existingConfig = file_exists($cfgPath) ? @include $cfgPath : [];
$hasConfiguredDb = is_array($existingConfig) && !empty($existingConfig['db']['name']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($hasConfiguredDb) && empty($_POST['confirm_existing_config_install'])) {
        $error = 'Mevcut config dolu görünüyor. Canlı veriyi korumak için kurulum durduruldu. Fresh install yapmak istiyorsanız onay kutusunu işaretleyin.';
    } else {
...
<title>Ahost One v25.0.0 RC20 - Kurulum</title>
<style>
...
<div class="box">
<div class="box-head">
<h1>🚀 Ahost One v25.0.0 RC20 Kurulum</h1>
...
<?php if (!empty($hasConfiguredDb) && $_SERVER['REQUEST_METHOD'] !== 'POST'): ?>
<div class="err"><strong>Dikkat:</strong> Bu paket zaten yapılandırılmış bir config dosyası içeriyor. Canlı sistemde müşteri/içerik verilerini korumak için install.php çalıştırmayın; dosyayı <code>install.php.disabled</code> olarak yeniden adlandırın. Fresh install sadece boş/yeni veritabanı içindir.</div>
<?php endif; ?>
<form method="post">
<div class="section">
<h2>1. Veritabanı</h2>
...
<?php if (!empty($hasConfiguredDb)): ?>
<div class="full" style="margin:14px 0;padding:14px;border:2px solid #fbbf24;border-radius:12px;background:#fffbeb;color:#92400e">
<label style="display:flex;gap:10px;align-items:flex-start;color:#92400e"><input type="checkbox" name="confirm_existing_config_install" value="1" style="width:auto;margin-top:2px"> Bu sistemde dolu config var. Mevcut veritabanına kurulum yapmanın canlı verileri etkileyebileceğini anlıyorum.</label>
</div>
<?php endif; ?>
