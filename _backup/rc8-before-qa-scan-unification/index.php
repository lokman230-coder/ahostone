...
if ($route === 'admin/scan-report' || $route === 'admin/scan-report/run') {
    require_admin();
    $scan = ao_run_full_scan();
    $_SESSION['ao_last_scan'] = $scan;
    view('scan-report/index', ['pageTitle'=>'Scan & Report Center', 'scan'=>$scan]); exit;
}
if ($route === 'admin/scan-report/pdf') {
    require_admin();
    $scan = $_SESSION['ao_last_scan'] ?? ao_run_full_scan();
    $lines = ['Tarih: '.$scan['generated_at'], 'Genel Skor: '.$scan['score'].'/100', ''];
    foreach ($scan['rows'] as $r) $lines[] = strtoupper($r['status']).' | '.$r['category'].' | '.$r['name'].' | '.$r['detail'];
    header('Content-Type: application/pdf');
    header('Content-Disposition: attachment; filename="ahost-one-scan-report.pdf"');
    echo ao_build_simple_pdf('Ahost One Sistem Tarama Raporu', $lines);
    exit;
}
...
// v25.0.0 RC7 QA Visual Scan report generator
if ($_SERVER['REQUEST_METHOD']==='POST' && $route==='admin/qa-visual-scan/run') {
    require_admin(); verify_csrf();
    require_once __DIR__.'/app/Services/QAVisualScanService.php';
    $base = rtrim(url(''), '/');
    try {
        $dir = QAVisualScanService::createReport($base);
        flash('success', 'QA görsel tarama raporu oluşturuldu: '.basename($dir));
    } catch (Throwable $e) {
        flash('error', 'QA raporu oluşturulamadı: '.$e->getMessage());
    }
    redirect_to('admin/qa-visual-scan');
}
...
$adminMap = [
 ...
 'admin/api-integrations' => 'api-integrations/index','admin/qa-visual-scan'=>'qa-visual-scan/index','admin/module-center'=>'module-center/index','admin/provider-center'=>'provider-center/index','admin/currency-center'=>'currency-center/index','admin/translation-center'=>'translation-center/index',
