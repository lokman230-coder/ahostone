<?php
/**
 * Theme Marketplace Pro
 * Admin temalari ve landing page sablonlari - ZIP yukleme, lisans, guncelleme
 */
namespace Ahost\Modules\ThemeMarketplace;
...
class ThemeMarketplaceModule {
    ...
    public static function install(\PDO $db): bool {
        $db->query("
            CREATE TABLE IF NOT EXISTS `theme_marketplace_products` (
        ...
        $db->query("
            CREATE TABLE IF NOT EXISTS `theme_marketplace_licenses` (
                `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        $db->query("
            CREATE TABLE IF NOT EXISTS `theme_marketplace_orders` (
                `id` INT AUTO_INCREMENT PRIMARY KEY,
                `product_id` INT NOT NULL,
                `seller_id` INT NOT NULL,
                `buyer_id` INT NOT NULL,
        ...
        $db->query("
            CREATE TABLE IF NOT EXISTS `theme_marketplace_updates` (
                `id` INT AUTO_INCREMENT PRIMARY KEY,
                `product_id` INT NOT NULL,
    ...
    public static function validateLicense($license_key, $domain = null) {
        ...
        $stmt = $db->prepare("
            SELECT tml.*, tmp.name as product_name, tmp.license_type
            FROM theme_marketplace_licenses tml
            JOIN theme_marketplace_products tmp ON tmp.id = tml.product_id
            WHERE tml.license_key = ?
        ");
